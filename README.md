...
　
#### 各エンドポイントとそれぞれの内容について
| エンドポイント | 説明                     | 主なクエリパラメータ      | レンダリング内容               |
| -------------- | ------------------------ | ------------------------- | ---------------------------- |
| `/janken`      | じゃんけんの結果を表示   | `hand`, `win`, `total`    | `hand`、`cpu`、`judgement`、`win`、`total` |
| `/lottery`     | ランダムな宝くじ番号の結果を表示 | なし                      | `number`、`item`               |
| `/gatya`       | ランダムなガチャ結果を表示 | `times`（デフォルトは1回）| リスト`results`           |
